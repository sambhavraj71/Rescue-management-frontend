...
export default function RescueScreen({ route }) {
  ...
  const handleAccept = async (sosId) => {
    try {
      ...
      Alert.alert("✅ Accepted", "You have accepted this SOS. Navigating to victim.");
  ...
  const handleComplete = async (sosId) => {
    Alert.alert("Mark Complete?", "Confirm rescue operation is completed.", [
  ...
  const renderDashboard = () => (
    <ScrollView contentContainerStyle={styles.dashContainer}>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={[styles.statBox, { borderColor: "#64748b" }]}>
      ...
      {item.statusHistory?.length > 0 && (
        <View style={styles.historyBox}>
          {item.statusHistory.slice(-2).map((h, i) => (
            <Text key={i} style={styles.historyItem}>• {h.status} — {h.note}</Text>
          ))}
        </View>
      )}
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.tabBar}>
        {[
          ...
          { id: "mycases", label: `My Cases (${myActive.length})`, icon: "shield-checkmark" },
        ].map((t) => (
